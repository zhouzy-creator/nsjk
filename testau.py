...
def policy_vtest():
    """ (policy, pro, level)"""
    policy = [
        [('Flip', 1, 10)], [('Rotate_BBox', 0.6, 10), ('Color', 1.0, 6)]
    ]
    return policy
...
def _shift_bbox(bbox, image_height, image_width, pixels, shift_horizontal):
    ...
    print("_shift_bbox",bbox,image_height, image_width, pixels,shift_horizontal)
    ...
    print("shift end",min_x, min_y, max_x, max_y)
...
def _apply_func_with_prob(func, image, args, prob, bboxes):
    ...
    print(inspect.getfullargspec(func)[0][1],inspect.getfullargspec(func))
...
def apply_policy(policy_name, image, bboxes,args):
    """Select a random policy from `policies` and apply it to `image`."""
    print(policy_name)
    #policy_info = list(policy_info) + [replace_value, augmentation_hparams]
    policy= NAME_TO_FUNC[policy_name]
    # Note that using tf.case instead of tf.conds would result in significantly
    # larger graphs and would even break export for some larger policies.

    #print(len(args))
    args =args[1:]
    print(args,args[0],policy)
    if('bboxes' == inspect.getfullargspec(policy)[0][1]):
        image, bboxes = _apply_func_with_prob(policy,image,args=args,prob=1,bboxes=bboxes)
    else:
        image = policy(image,  args[0])

    return (image, bboxes)
def _scale_bbox(box,ih,iw):
...
def _norm_bbox(box,ih,iw):

    min_x = float(box[0]/iw)
    min_y = float(box[1]/ih)
    max_x = float(box[2]/iw)
    max_y = float(box[3]/ih)
...
class AutoAugmenter(object):
    ...
    def write_xml(self,xml_name,new_name,h,w,boxs,classlist):
        ...
        for j, box in enumerate(boxs):
            ...
            #         child4.append(pose)
            #         child4.append(truncated)
            #         child4.append(difficult)
    ...
    def __call__(self, xml_name,image_name):
        #image_name, annots = sample['img'], sample['annot']
        # annots = self.normalizer(image, annots)
        # bboxes = annots[:, 0:4]
        # image = np.uint8(image * 255)

        autolist=[("Flip",1),("TranslateX_BBox",1,8),("Flip_Only_BBoxes",10),
                  ('BBox_Cutout', 0.2, 10),('Sharpness',  8),('Rotate_BBox', 0.6, [1]),
                        ('Cutout',  4), ('Color',  3), ('ShearX_BBox', 0.4, 1)
                  ]
        for autuple in autolist:
            bboxes, classes = self.parse_xml(xml_name)
            image = np.array(Image.open(image_name))
            image_height = image.shape[0]
            image_width = image.shape[1]

            normial_bbox = lambda bbox: _norm_bbox(bbox, image_height, image_width)
            bboxes = np.array([box for box in list(map(normial_bbox, bboxes)) if box is not None])

            autoname = autuple[0]
            image, bboxes = apply_policy(autoname,image, np.array(bboxes),autuple)
            p, i_name= os.path.split(image_name)
            p, x_name = os.path.split(xml_name)
            auimage = Image.fromarray(np.uint8(image))
            new_iname= os.path.join(global_config.AU_PATH,autoname+i_name)
            new_xname = os.path.join(global_config.AU_PATH,autoname+x_name)
            auimage.save(new_iname)
            print(" ebd ",image.shape[0],image.shape[1] )
            rescale_bbox = lambda bbox: _scale_bbox(bbox, image_height, image_width)
            # 格式正确
            # pylint:enable=g-long-lambda
            bboxes = np.array([box for box in list(map(rescale_bbox, bboxes)) if box is not None])
            self.write_xml(new_xname,new_iname,image.shape[0],image.shape[1],bboxes,classes)
